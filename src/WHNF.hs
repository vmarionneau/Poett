...
instance Show STm where
  ...
  show (Let tm1 tm2) = "let " ++ show tm1 ++ " in " ++ show tm2
  show (Ident s) = s
...
bump n (U l) = U l
bump n (Pi ty tm) = Pi (bump (n + 1) ty) (bump n tm) 
bump n (Abs ty tm) = Abs (bump (n + 1) ty) (bump n tm) 
bump n (App tm1 args) = App (bump n tm1) (bump n <$> args)
bump n (Let tm1 tm2) = Let (bump n tm1) (bump n tm2)
bump n (Ident i) = Ident i
bump n (Constr ind i args) = Constr ind i (bump n <$> args)
bump n (Elim ind) = Elim ind
bump n (ESubst s tm) = ESubst (Subst (bump n <$> vars s) (idents s)) (bump n tm)

lift :: Subst → Subst
lift s = Subst (Var 0 : (bump 0 <$> vars s)) (idents s)
...
execSubst t = aux (Subst [] Map.empty) t
  where
    ...
    aux s (Let tm1 tm2) = Let (aux s tm1) (aux (lift s) tm2)
    aux s (Ident i) = fromMaybe (Ident i) (idents s Map.!? i)
...
erase (Let tm1 tm2) = Let (erase tm1) (erase tm2)
erase (Constr ind i args) =
  Constr (Ind (indName ind) [] (Arity [] Prop) []) i (erase <$> args)
erase (Elim ind) = Elim (Ind (indName ind) [] (Arity [] Prop) []) 
erase t = t
